...
class FF_Dataset_Nolabel_Unnormalized_Augmented(torch.utils.data.Dataset):
    '''
    ...
    def __init__(self, opt, partition):
        self.opt = opt
        original_cifar10 = utils.get_CIFAR10_partition(opt, partition)
        if partition == "train":
            augmented_cifar10 = utils.get_CIFAR10_train_unnormalized_augmented(opt) 
